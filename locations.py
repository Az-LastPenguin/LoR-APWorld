...
def setup_locations(random: random.Random, options: LOROptions):
    ...
    if options.abno_randomization == 1:
        for f in run_floors:
    ...
    if options.receptions_progression == 2 or options.floor_progression == 1:
...
def generate_reception_tree(random: random.Random):
    ...
    for i in range(1, height):
        ...
        for node in nodes[i]:
            ...
            while True:
                candidate = candidates.pop(random.randint(0, len(candidates) - 1) if len(candidates) > 1 else 0)
